                        ...
                        <div key={member.id} className="group relative bg-zinc-900 border border-zinc-800 rounded-3xl p-6 hover:border-zinc-600 transition-all duration-300 shadow-sm hover:shadow-2xl flex flex-col justify-between h-full min-h-[340px]">
                            ...
                            <div className="flex flex-col items-center gap-5 mt-4">
                                <div className="relative">
                                    ...
                                    <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 whitespace-nowrap px-3 py-1 rounded-full bg-zinc-950 border border-zinc-800 text-[10px] font-medium text-yellow-500 font-unbounded capitalize shadow-lg">{member.role}</div>
                                </div>
                                <div className="text-center pt-2 px-2">
                                    <h3 className="text-md font-bold text-white leading-tight font-unbounded mb-1">{member.full_name}</h3>
                            ...
                            <div className="py-6 w-full">
                                <button
                                    ...
                                    className={`w-full flex items-center font-unbounded font-normal text-xs justify-center gap-2 border border-zinc-800 py-2.5 rounded-xl transition-all group/btn cursor-pointer
                    ...
                    `}
                            ...
                            <div className="pt-4 border-t border-zinc-800/50 flex items-center justify-center w-full mt-auto">
                                <div className="flex items-center gap-2.5 bg-zinc-950/50 px-3 py-1.5 rounded-full border border-white/5">
                                    ...
                                    <span className={`text-[10px] font-semibold tracking-wide font-unbounded ${member.is_active ? 'text-zinc-300' : 'text-zinc-500'}`}>{member.is_active ? 'Activo' : 'Inactivo'}</span>
                                </div>
                            </div>
                        </div>
                    );
                ...
                {currentUserRole === 'admin' && (
                    ...
                    >
                        ...
                        <div className="text-center space-y-1">
                            <span className="block text-md font-unbounded font-semibold text-zinc-400 group-hover:text-white transition-colors">Añadir compañero</span>
                        </div>
                    </button>
                )}
            </div>
            ...
            {isEditModalOpen && editingMember && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 stagger-container">
                    <div className="bg-zinc-900 border border-zinc-800 rounded-3xl w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]">
                        <div className="flex items-center justify-between px-6 py-5 border-b border-zinc-800 bg-zinc-900/50 rounded-t-3xl">
                            <div>
                                <h3 className="text-md font-unbounded font-bold text-white">Editar Perfil</h3>
                        ...
                        <form action={editAction} className="flex flex-col h-full overflow-hidden">
                            <div className="p-6 space-y-6 overflow-y-auto custom-scrollbar">
                                ...
                                <div className="space-y-2">
                                    <label className="flex items-center gap-2 text-xs font-semibold text-zinc-500 font-unbounded"><User className="w-3 h-3" /> Nombre Completo</label>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        <label className="flex items-center gap-2 text-xs font-semibold text-zinc-500 font-unbounded"><Mail className="w-3 h-3" /> Email</label>
                                    ...
                                    <div className="space-y-2">
                                        <label className="flex items-center gap-2 text-xs font-semibold text-zinc-500 font-unbounded"><Shield className="w-3 h-3" /> Rol</label>
                                ...
                                <div className="space-y-2">
                                    <label className="flex items-center gap-2 text-xs font-semibold text-zinc-500 font-unbounded"><FileText className="w-3 h-3" /> Descripción</label>
                                ...
                                <div className={`bg-zinc-950/50 border border-zinc-800 rounded-xl p-4 flex items-center justify-between ${!canEditStatus ? 'opacity-50' : ''}`}>
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h4 className="text-sm font-semibold font-unbounded text-white">Estado de la cuenta</h4>
                            ...
                            <div className="px-6 py-4 bg-zinc-950 border-t border-zinc-800 flex gap-3 mt-auto rounded-b-3xl">
                                <button type="button" onClick={() => setIsEditModalOpen(false)} className="flex-1 py-3 text-sm rounded-xl border border-zinc-800 text-zinc-400 hover:text-white font-semibold font-unbounded cursor-pointer">Cancelar</button>
                                <SaveEditButton />
                            </div>
                        </form>
                    </div>
...
function SaveEditButton() {
    const { pending } = useFormStatus()
    return (
        <button type="submit" disabled={pending} className="flex-1 py-3 rounded-xl bg-yellow-500 text-zinc-950 font-semibold font-unbounded text-sm flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed">
